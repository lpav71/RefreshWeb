...
class TaskCommon
{
    public static function getAll($club_id, $admin_id)
    {
        $tasks = Task::where('task.admin_id', $admin_id)
            ...
            ->select('task.descript_admin', 'task.id', 'task.create_dt', 'task.end_dt','users.name','employ.name as employ','users.name','task_description.description', 'task_description.color', 'task_description.id as id_desk')
            ->orderBy('id')
            ->get();
        return $tasks;
    }
