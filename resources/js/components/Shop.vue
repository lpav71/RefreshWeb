<template>
        ...
        <div class="left">
            <div class="user-select" style="">
                ...
                <div style="margin-top: -33px;">
                    ...
                    <input v-model="searchQuery" class="select_user" placeholder="Поиск логина" type="text" @input="searchClients" @keydown="searchClientToKey"/>
                </div>
                <div v-show="findWindow" ref="findBlock" class="popup">
                    <ul>
                        <li v-for="(f, i) in filteredClients" @click="selectFindClient(i)">{{ f.login }}</li>
                    </ul>
                </div>
                <div class="user_name">
                    <img alt="" src="images/shop/ava.svg" style="width: 22px;margin-top: 10px;margin-left: 10px;"/>
                ...
                <div v-if="findClientStatus" class="info">
                    <img alt="" height="16" src="images/shop/rub.png" style="margin-top: 12px;margin-left: 17px;" width="16"/>
                    <span class="rub_summa">{{ findClient.amount }}</span>
                    <img alt="" height="16" src="images/shop/coin.png" style="margin-top: 10px;margin-left: 97px;" width="16"/>
                    <span class="rub_summa" style="color: #ffcd00;">{{ findClient.bonus }}</span>
                </div>
            </div>
            <button class="btn btn-success" style="width: 250px;" type="button" @click="clearClient">Нет логина [гость]</button>
            <span style="margin-bottom: -24px;margin-top: 8px;">Корзина</span>
            <div class="cart">
                <div v-for="(c, index) in cart" :class="{'goods-discount': c.allowDiscount}" class="cart_element">
            ...
            <div class="bottom3"><span>Применить промокод</span>
                <input v-model="promo_code" class="user_name" placeholder="refresh 2023" style="text-align: center;" type="text"/>
            ...
            <button class="btn btn-danger mt-4" style="width: 250px;" type="button" @click="pay">Оплатить</button>
        </div>
        <div class="right">
            <div class="top">
                <img alt="" src="images/shop/2.svg"/><span style="margin-left: 14px;">Магазин</span><i class="fa fa-search lupa"></i><input v-model="product_name" class="input_shop" placeholder="Поиск товаров" type="text" @keydown="enter"/>
            ...
            <div class="fon">
                <div class="bottom2" style="border-radius: 20px;padding: 30px;">
                    <div v-for="(g, index) in goods" class="goods">
                        <div class="goods_img"><img :src="g.icon" alt="" height="170"/></div>
                        <div class="info-block">
                            <div class="goods_text">
                                <div>
                                    <div style="opacity: 0.5">{{ g.name }}</div>
                                    <div style="font-size: 12px;height: 17px;overflow: hidden;">{{ g.product }} {{ g.product_param }}</div>
                                </div>
                                <div v-if="g.price != 0" style="margin-top: 25px"><span>{{ g.price }} Р</span></div>
                            </div>
                            <div class="button-block">
                                <button class="goods_cart" style="background: linear-gradient(180deg, rgba(40, 79, 98, 0.05) 0%, #183f52 100%);margin-bottom: 3px;" type="button" @click="toCartPlus(index)">
                                    <div class="pm" style="position: relative;right: 2.4px;">+</div>
                                </button>
                                <button class="goods_cart" type="button" @click="toCartMinus(index)">
                                    <div class="pm" style="position: relative;bottom: 4px;">-</div>
                                </button>
                            </div>
                        </div>
                    </div>
    ...
    <!-- Модальное окно -->
    <div id="payModal" aria-hidden="true" aria-labelledby="payModalLabel" class="modal fade" tabindex="-1">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content content">
                <div class="modal-header">
                    <h5 id="payModalLabel" class="modal-title">Оплата</h5>
                            ...
                            <div class="div_data">
                                <div class="div_3">
                                    <div>
                                        ...
                                        <div class="div_4">
                                            <input id="formcheck_1" checked class="formcheck_1" name="pay" type="radio"/><label class="form-label" for="formcheck_1">Наличные</label>
                                        </div>
                                        <div class="div_4">
                                            <input id="formcheck_2" class="formcheck_1" name="pay" type="radio"/><label class="form-label" for="formcheck_2">Карта</label>
                                        </div>
                                    </div>
                                </div>
                                <div class="div_3">
                                    <div>
                                        <span>Чек</span>
                                        <div class="div_4">
                                            <input id="formcheck_3" class="formcheck_1" type="checkbox"/><label class="form-label" for="formcheck_3">Отправить на почту</label>
                                        </div>
                                        <div class="div_4">
                                            <input id="formcheck_4" class="formcheck_1" type="checkbox"/><label class="form-label" for="formcheck_4">Печать</label>
                                        </div>
                                    </div>
                                </div>
                            </div>
                ...
                <div class="modal-footer">
                    <button class="btn btn-primary save" type="button" @click="payJson">Оплата</button>
                </div>
            </div>
        </div>
    </div>
...
export default {
    ...
    methods: {
        ...
        payJson() {
            ...
            createLog(json_data, "Покупка в магазине", "shop", this.$props.user_id);
            this.cart          = [];
            this.fullPromoCode = {};
            this.promo_code    = '';
            this.verifyPromo   = false;
