    ...
    <div class="account">
        <form action="finduser" method="get">
            <div class="left-user-account"><i class="fa fa-search fs-5 lupa"></i>
                <input v-model="searchQuery" class="user-search" name="searchUser" placeholder="Поиск пользователя" type="text" @input="searchClients">
            <div v-if="findWindow" ref="findBlock" class="popup">
                <ul>
                    <li v-for="(f, i) in filteredClients" @click="selectFindClient(i)">{{ f.login }}</li>
                </ul>
            </div>
        </div>
        <button ref="send" style="display: none" type="submit"></button>
        </form>
        <div class="right-user-account">
            ...
            <div class="user-select-button">
                <div class="avatar"><img :src="icon" alt="" style="width: 42px;height: 42px;"></div>
